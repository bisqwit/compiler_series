...
struct statement
{
    ...
    bool ForAllRegs(F&& func, std::size_t begin=0, std::size_t end = ~size_t())
    {
        return std::any_of(params.begin()+begin, params.begin()+std::min(end,params.size()),
                           [&](reg_type& p) { return p < no_mask && callv(func,false,p, &p-&params[0]); });
    }
...
void statement::Dump(std::ostream& out) const
{
    ...
    {
        if(u >= no_mask)
            { for(const auto& p: number_constants)
                  if(p.second == u)
                      { if(p.first.first.empty()) out << ' ' << p.first.second;
                        else if(!p.first.second) out << " \"" << p.first.first << '"';
                        else out << " \"" << p.first.first << "\"+" << p.first.second; } }
        else
            out << " R" << u;
    }
...
struct compilation
{
    ...
    bool Optimize_ConvertLiterals()
    {
        ...
        auto MakeRegFromLiteral = [&](std::pair<std::string,long>&& value)
        {
            ...
            auto i2 = number_constants.emplace(std::move(value), r);
                        ...
                        if(p.presence[wregno2] == p2.presence[wregno2])
                        {
                            ...
                            for(const auto& consumer: p.params[windex])
                                if(auto reader = is_statement(consumer))
                                    if(reader != write_insn2 && !is_fcall(*reader)/*&& !reader->HasSideEffects()*/ && !is_ifnz(*reader) && !is_ret(*reader)
                                      ...
                                      )
                                    {
                                        ...
                                        }))
                                        {
                                            ...
                                            for(auto& s: all_statements) { RePoint(s->next); if(is_ifnz(*s)) RePoint(s->cond); }
