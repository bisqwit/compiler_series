...
std::string stringify(const expression& e, bool tree, bool stmt);
std::string stringify_op(const expression& e, const char* sep, const char* delim, bool tree, bool stmt = false, unsigned first=0, unsigned limit=~0u)
{
    std::string result; if(delim) result += delim[0];
    const char* fsep = "";
    for(const auto& p: e.params) { if(first) { --first; continue; }
                                   if(!limit--) break;
                                   result += fsep; fsep = sep; result += stringify(p, tree, stmt); }
    if(stmt) result += sep;
    if(delim) result += delim[1];
    return result;
}
std::string stringify_opt_parens(const expression& e, bool tree, bool stmt = false)
{
    if(is_ident(e)) return stringify(e, tree, stmt);
    return "(" + stringify(e, tree, stmt) + ")";
}
std::string stringify(const expression& e, bool tree, bool stmt = false)
{
    auto expect1 = [&](bool st=false){ return e.params.empty() ? "?" : e.params.size()==1 ? stringify(e.params.front(),tree,st) : stringify_op(e, "??", "()", tree); };
    switch(e.type)
    {
        // atoms
        ...
        case ex_type::ident  : return tree ? "?FPVS"[(int)e.ident.type] + std::to_string(e.ident.index) + "\"" + e.ident.name + "\""
                                           : e.ident.name;
        // binary & misc
        case ex_type::add    : return stringify_op(e, "+",  stmt?nullptr:"()", tree);
        case ex_type::eq     : return stringify_op(e, "==", stmt?nullptr:"()", tree);
        case ex_type::cand   : return stringify_op(e, "&&", stmt?nullptr:"()", tree);
        case ex_type::cor    : return stringify_op(e, "||", stmt?nullptr:"()", tree);
        case ex_type::comma  : return stmt ? stringify_op(e, ";", "{}", tree, true) : stringify_op(e, ",",  "()", tree);
        // unary
        case ex_type::neg    : return "-(" + expect1() + ")";
        case ex_type::deref  : return "*(" + expect1() + ")";
        case ex_type::addrof : return "&(" + expect1() + ")";
        // special
        case ex_type::copy   : if(stmt) return stringify(e.params.back(),tree) + " = " + stringify(e.params.front(),tree);
                               return "(" + stringify(e.params.back(),tree) + " = " + stringify(e.params.front(),tree) + ")";
        case ex_type::fcall  : return (e.params.empty() ? "?" : stringify_opt_parens(e.params.front(),tree))+stringify_op(e,",","()",tree,false,1);
        case ex_type::loop   : return "while " + stringify(e.params.front(),tree) + " " + stringify_op(e, ";", "{}", tree, true, 1);
        case ex_type::ret    : return "return " + expect1(true);
    }
    return "?";
}
static std::string stringify(const function& f)
{
    return stringify(f.code,false,true);
}

#include "textbox.hh"

static std::size_t beautify(std::vector<std::string>& result,
                            const std::string& s, std::size_t startpos,
                            std::size_t linenumber, unsigned indent)
{
    auto line = [&]() -> std::string&
    {
        while(result.size() <= linenumber) result.emplace_back();
        std::string& line = result[linenumber];
    ...
    while(startpos < s.size())
    {
        ...
        switch(s[startpos])
        {
            ...
            case '(':
            {
                std::size_t subindent = 0;
                while(startpos+1 < s.size() && (s[startpos+1] == ',' || s[startpos+1] == ';'))
                {
                    line() += s[++startpos];
                }
            loop:
                while(startpos+1 < s.size() && (s[startpos+1] == ' '))
                {
                    line() += s[++startpos];
                }
                if(!subindent) subindent = line().size();
                startpos = beautify(result, s, startpos+1, linenumber, subindent);
            ...
            chk:
                switch(s[startpos])
                {
                    ...
                    case ')':
                        ...
                        break;
                    case ',':
                        if(line().size() >= 40) ++linenumber; else { line() += ' '; }
                        goto loop;
                    case ';':
                        while(s[startpos+1] == ';' || s[startpos+1] == ' ')
                        {
                            line() += s[++startpos];
                        }
                        if(startpos >= s.size()) break;
                        if(s[startpos+1] == '}')
                        {
                            line() += s[++startpos];
                            ++startpos;
                        ...
                        {
                            std::string nextword;
                            for(std::size_t a=startpos+1; a<s.size(); nextword += s[a++])
                                if(!std::isalnum(s[a]))
                                    break;
                            if(line().size() >= 40 || nextword.size()>5 || nextword=="while")
                                ++linenumber;
                            else { line() += ' '; }
                            goto loop;
                        }
...
static std::vector<std::string> beautify(const std::string& s)
{
    std::vector<std::string> result;
    beautify(result, s, 0, 0, 0);
    return result;
}

static textbox stringify_tree(const function& f)
{
    textbox result;
    result.putline("\201function " + f.name + ':', 0,0);
    std::size_t y = 1;
    //result.putline("`" + stringify(f), 0,y++);
    std::vector<std::string> lines = beautify(stringify(f));
    for(std::size_t a=0; a<lines.size(); ++a)
        result.putline("\202" + lines[a], 0, y++);
    result.putbox(2,y, create_tree_graph(f.code, 132-2,
        [](const expression& e)
        {
            std::string p = stringify(e,true), k = p;
            switch(e.type)
            {
...
struct compilation
{
    ...
    private:
        void Trace(statement* where, StateType&& state, bool follow_copies, bool include_ifnz_as_writer)
        {
            ...
            else
            {
                ...
                where->ForAllWriteRegs([&](auto wregno, auto) { state[wregno] = { where }; });
                ...
                if(include_ifnz_as_writer && is_ifnz(*where)) { state[where->lhs()] = { where }; };
            }
    ...
    statement::reg_type Compile(const expression& code, compilation_context& ctx)
    {
        ...
        switch(code.type)
        {
            ...
            case ex_type::addrof: std::cerr << "NO IDEA WHAT TO DO WITH " << stringify(code,false) << '\n'; break; // Unhandleable
...
int main(int /*argc*/, char** argv)
{
    ...
    for(const auto& f: func_list) { auto m = stringify_tree(f); before.push_back(m); /*std::cerr << m.to_string();*/ }

    DoConstantFolding();

//    std::cerr << "Final\n";
    for(const auto& f: func_list) { auto m = stringify_tree(f); after.push_back(m);  /*std::cerr << m.to_string();*/ }
    ...
    for(std::size_t a=0; a<before.size(); ++a)
    {
        ...
        if(a > 0)
        {
            std::size_t y1 = left.vert_append_position(0, before[a]);
            std::size_t y2 = right.vert_append_position(0, after[a]);
            y = std::max(y1, y2);
        }
        left.putbox(0, y, before[a]);
        //right.putbox(0, y, after[a]);
    }
    std::size_t x = left.horiz_append_position(0, right);
    for(std::size_t y=std::max(left.height(), right.height()); y--> 0; )
        if(left.getchar(x+1,y) == ' ')
            left.putchar('\200', x+1,y);
    left.putbox(x+2, 0, right);

    compilation code;
    code.Compile();

    unsigned tab1=10, tab2=tab1+5;
    //std::cerr << "COMPILED CODE\n";
    //code.Dump(std::cerr);
    std::stringstream str_unopt; code.Dump(str_unopt);
    textbox right2;
    {std::string s;
    unsigned x=0, y=0;
    while(std::getline(str_unopt,s,'\n'))
    {
        right2.putchar('\204', x++, y);
        for(char c: s)
            if(c == '\t') { if(x<4) x = tab1; else x = tab2; }//unsigned tab = x ? 6 : 5; x = (x+tab) - (x+tab)%tab; }
            else right2.putchar(c, x++, y);
        if(!s.empty() && s[s.size()-1]==':') { x = 0; } else { ++y; x=0; }
    }}
    ...
    //code.Dump(std::cerr);
    std::stringstream str_opt; code.Dump(str_opt);
    textbox right3;
    {std::string s;
    unsigned x=0, y=0;
    while(std::getline(str_opt,s,'\n'))
    {
        right3.putchar('\204', x++, y);
        for(char c: s)
            if(c == '\t') { if(x<4) x = tab1; else x = tab2; }//unsigned tab = x ? 6 : 5; x = (x+tab) - (x+tab)%tab; }
            else right3.putchar(c, x++, y);
        if(!s.empty() && s[s.size()-1]==':') { x = 0; } else { ++y; x=0; }
    }}

    {auto& box = right2; std::size_t x = left.horiz_append_position(0, box);
    for(std::size_t y=std::max(left.height(), box.height()); y--> 0; )
        if(left.getchar(x+1,y) == ' ')
            left.putchar('\200', x,y);
    left.putbox(x+1, 0, box);}

    {auto& box = right3; std::size_t x = left.horiz_append_position(0, box)+1;
    for(std::size_t y=std::max(left.height(), box.height()); y--> 0; )
        if(left.getchar(x+1,y) == ' ')
            left.putchar('\200', x,y);
    left.putbox(x+1, 0, box);}
